...
async function combineInSink (sink, assets, quadFn = (quad) => quad) {
  ...
  assets.forEach(({ dataset }) => {
    dataset?.forEach(quad => combinedStream.push(quadFn(quad)))
  })
...
async function toTurtleStream (assets, prefixes = {}) {
  ...
  const quadFn = (quad) => rdf.quad(quad.subject, quad.predicate, quad.object) // Turtle does not serialize named-graphs
  return await combineInSink(turtleSink, assets, quadFn)
}
